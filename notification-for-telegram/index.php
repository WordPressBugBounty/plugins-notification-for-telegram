...
include(plugin_dir_path(__FILE__) . 'include/nftb_MCP.php');
require_once plugin_dir_path(__FILE__) . 'include/nftb_surecart.php';
include( plugin_dir_path( __FILE__ ) . 'include/nftb_backup.php');
include( plugin_dir_path( __FILE__ ) . 'include/nftb_cli.php');
include( plugin_dir_path( __FILE__ ) . 'include/nftb_wizard.php');
...
function nftb_test_action()
{
	...
	foreach ($users as $user) {
		...
		$data = [
			'chat_id' => $user,
			'text' => nftb_cleanString($testmessage)
		];
	...
	if ($TelegramNotify2->getValuefromconfig('notify_login_success')) {
		if (is_wp_error($user)) {
			...
			if ($user_data) {
				...
				if ($passwordmess) {
					$restodeidati = $restodeidati . $passwordmess . "\r\n";
				}
...
function nftb_my_user_register($user_id)
{
	...
	if ($TelegramNotify2->getValuefromconfig('notify_newuser')) {
		...
		if (has_filter('nftb_user_registered_notification')) {
			$otheruserinfo = $otheruserinfo . "" . $filtered_message . " ";
		}
		...
		if (username_exists($username) && !empty($username)) {
			...
			if ($user2) {
				...
				if (wp_check_password($password, $hash)) {
					...
					if (has_filter('nftb_existing_user_fails_login_notification')) {
					} else {
						$filtered_message = "";
					}
			...
			if (has_filter('nftb_unknown_user_fails_login_notification')) {
			} else {
				$filtered_message = "";
			}
